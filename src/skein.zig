...
fn load64le(p: []const u8) u64 {
    return @as(u64, p[0]) | (@as(u64, p[1]) << 8) | (@as(u64, p[2]) << 16) |
           (@as(u64, p[3]) << 24) | (@as(u64, p[4]) << 32) | (@as(u64, p[5]) << 40) |
           (@as(u64, p[6]) << 48) | (@as(u64, p[7]) << 56);
}
...
pub fn skein512Init(ctx: *Skein512Ctxt, hashBitLen: usize) !void {
    ...
    cfg[4] = 1; cfg[5] = 0;
    var ob = hashBitLen;
    for (0..8) |i| { cfg[8 + i] = @as(u8, @truncate(ob)); ob >>= 8; }
...
// -----------------------------------------------------------------------
// Self-test: Skein-1.3 Appendix B vectors stored as byte arrays.
// No hex string parsing -- no opportunity for nibble-alignment bugs.
// -----------------------------------------------------------------------
...
const kat2_in  = [32]u8{ 0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f,
